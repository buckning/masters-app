...
//        try{
//        	bufferedGraphics.drawImage(ImageIO.read(new File("example.jpg")),null,null);
//        }
//        catch(Exception e){
//        	e.printStackTrace();
//        }
//        bufferedGraphics.setColor(Color.black);
        bufferedGraphics.fillRect(0, 0, 1024, 768);
        for (int x = 0; x < data.length; x++)
        {
            for (int y = 0; y < data[0].length; y++)
            {
//            	colors[dataColorIndices[x][y]] = new Color(rgb)
//            	Color c = colors[dataColorIndices[x][y]];
//            	     
//            	Color a = new Color(c.getRed(), c.getGreen(), c.getBlue(), 70);
//            	bufferedGraphics.setColor(a);
            	int r = colors[dataColorIndices[x][y]].getRed();
            	int g = colors[dataColorIndices[x][y]].getGreen();
            	int b = colors[dataColorIndices[x][y]].getBlue();
            	Color c = new Color(r,g,b,70);
                bufferedGraphics.setColor(c);//colors[dataColorIndices[x][y]]);
//                bufferedGraphics.setStroke(new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[]{3,1}, 0));
                if(data[x][y]!= 0.0)bufferedGraphics.fillRect(x, y, 10, 10);//only draw if data is there i.e. don't draw data for no interactions
//                bufferedGraphics.fillOval(x, y, 20, 20);
            }
        }
    }
